...
def assignments_by_student():
    ...
    studentid = request.args.get('studentid')   # register_number
    ...
    for row in cursor.fetchall():
        assignments.append({
            "id": row[0], # unique row id
            "assno": row[1],            
            "label": f"Assignment {row[1]}"   # show "Assignment 1", "Assignment 2", etc.
        })
    conn.close()
...
def get_pending_assignments():
    courseid = request.args.get("courseid")
    regno = request.args.get("register_no")
    conn = sqlite3.connect("users.db")
    cur = conn.cursor()

    cur.execute("""
        SELECT assno FROM student_assignment 
        WHERE register_no = ? AND courseid = ?
    """, (regno, courseid))
    submitted = {str(row[0]) for row in cur.fetchall()}  # make sure values are strings
    conn.close()

    all_assignments = {"1", "2"}
    pending = list(all_assignments - submitted)
...
def get_pending_practicals():
    ...
    # fetch submitted assno values for this student & course
    cur.execute("""
        SELECT assno FROM student_assignment
        WHERE register_no=? AND courseid=? AND assno IN (3,4)
    """, (regno, courseid))
    submitted = {row[0] for row in cur.fetchall()}

    pending = []
    if 3 not in submitted:
        pending.append(3)  # Internal practical
    if 4 not in submitted:
        pending.append(4)  # External practical
